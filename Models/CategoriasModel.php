<?php 
	...
	class CategoriasModel extends Mysql
	{
		...
		public function inserCategoria(string $nombre, string $descripcion, string $portada, int $status){
			...
			if(empty($request))
			{
				$query_insert  = "INSERT INTO categoria(nombre_categoria,descripcionC,portada,status) VALUES(?,?,?,?)";
		...
		public function updateCategoria(int $idcategoria, string $categoria, string $descripcion, string $portada, int $status){
			...
			$sql = "SELECT * FROM categoria WHERE nombre_categoria = '{$this->strCategoria}' AND id_categoria != $this->intIdcategoria";
			$request = $this->select_all($sql);

			if(empty($request))
			{
				$sql = "UPDATE categoria SET nombre_categoria = ?, descripcionC = ?, portada = ?, status = ? WHERE id_categoria = $this->intIdcategoria ";
				$arrData = array($this->strCategoria, 
								 $this->strDescripcion, 
								 $this->strPortada, 
								 $this->intStatus);
		...
		{
			$this->intIdcategoria = $idcategoria;
			$sql = "SELECT * FROM persona WHERE categoria_id = $this->intIdcategoria";
			$request = $this->select_all($sql);
			if(empty($request))
			{
